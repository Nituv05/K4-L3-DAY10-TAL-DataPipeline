...
def evaluate_pipeline(
    ...
    fallback_count = sum(
        "Fallback heuristic judge used" in item["judge"]["reasoning"] for item in answers
    )
    summary["judge_source"] = (
        "heuristic_fallback" if fallback_count == len(answers)
        else "mixed" if fallback_count else "llm"
    )
